		...
		*/
		...
		foreach($plugins as $plugin) {
			if(array_intersect(array("e"), str_split($plugin['type']))) {
				$data['site_url'] = $CONF['url']; $data['site_title'] = $settings['title']; $data['site_email'] = $CONF['email']; $data['plugin_chat'] = 1; unset($data['password']); unset($data['salted']);
...
class register {
	...
	function verify_if_email_exists($type = null) {
		...
		if($type) {
			$query = sprintf("SELECT `username`, `password` FROM `users` WHERE `email` = '%s' AND `suspended` != 2", $this->db->real_escape_string(mb_strtolower($this->email)));
		} else {
			$query = sprintf("SELECT `email` FROM `users` WHERE `email` = '%s' AND `suspended` != 2", $this->db->real_escape_string(mb_strtolower($this->email)));
		}
		$result = $this->db->query($query);
...
class logIn {
	...
	function in() {
		...
		if($user == 1) {
			...
			// Redirect the user to his personal profile
			header("Location: ".permalink($this->url."/index.php?a=feed"));
		} else {
...
class feed {
	...
	function getMessages($query, $type, $typeVal) {
		...
		foreach($rows as $row) {
			...
			if($row['type'] == 'shared') {
				...
				$shared_url = ($getPage['name'] ? permalink($this->url.'/index.php?a=page&name='.$getPage['name']) : permalink($this->url.'/index.php?a=profile&u='.$shared['username']));
			...
			$profile_url = ($row['idu'] == $this->page_data['by'] ? permalink($this->url.'/index.php?a=page&name='.$this->page_data['name']) : permalink($this->url.'/index.php?a=profile&u='.$row['username']));
			$profile_img = permalink(($row['idu'] == $this->page_data['by'] ? $this->url.'/thumb.php?t=a&w=50&h=50&src='.$this->page_data['image'] : $this->url.'/thumb.php?t=a&w=50&h=50&src='.$row['image']));
				...
				<div class="message-content">
					<div class="message-inner">
						...
						<div class="message-top">
							...
							<div class="message-time">
								<span id="time'.$row['id'].'"><a href="'.permalink($this->url.'/index.php?a=post&m='.$row['id']).'" rel="loadpage">
					...
					<div class="message-comment-box-container" id="comment_box_'.$row['id'].'"'.$style.'>
						<div class="message-reply-avatar">
							'.((!empty($this->user)) ? '<img src="'.permalink($this->url.'/thumb.php?t=a&w=50&h=50&src='.($this->id == $this->page_data['by'] ? $this->page_data['image'] : $this->user['image']).'">') : '').'
						</div>
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\''.$group['cover'].'\', \''.$group['id'].$group['title'].'\', \'covers\')" id="'.$group['cover'].'"><div class="cover-image" style="background-position: center; background-image: url('.permalink($this->url.'/thumb.php?t=c&w=900&h=200&src='.$group['cover']).')">
							</div></a>
	...
	function fetchPage($page) {
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\''.$page['cover'].'\', \''.$page['id'].$page['name'].'\', \'covers\')" id="'.$page['cover'].'"><div class="cover-image" style="background-position: center; background-image: url('.permalink($this->url.'/thumb.php?t=c&w=900&h=200&src='.$page['cover']).')">
							</div></a>
							<div class="cover-description">
								<div class="cover-avatar-content">
									<div class="cover-avatar">
										<a onclick="gallery(\''.$page['image'].'\', \''.$page['id'].$page['name'].'\', \'avatars\')" id="'.$page['image'].'"><span id="avatar'.$page['id'].$page['name'].'"><img src="'.permalink($this->url.'/thumb.php?t=a&w=150&h=150&src='.$page['image']).'"></span></a>
									</div>
								</div>
	...
	function fetchProfile($profile) {
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\''.$profile['cover'].'\', \''.$profile['idu'].$profile['username'].'\', \'covers\')" id="'.$profile['cover'].'"><div class="cover-image" style="background-position: center; background-image: url('.permalink($this->url.'/thumb.php?t=c&w=900&h=200&src='.$profile['cover']).'">
							</div></a>
							<div class="cover-description">
								<div class="cover-avatar-content">
									<div class="cover-avatar">
										<a onclick="gallery(\''.$profile['image'].'\', \''.$profile['idu'].$profile['username'].'\', \'avatars\')" id="'.$profile['image'].'"><span id="avatar'.$profile['idu'].$profile['username'].'"><img src="'.permalink($this->url.'/thumb.php?t=a&w=150&h=150&src='.$profile['image']).'"></span></a>
									</div>
								</div>
	...
	function coverButtons($type) {
		...
		foreach($buttons as $value => $name) {
			...
			if($value) {
				...
				$button .= '<a class="cover-button'.((($name[1] == $_GET['r'] && empty($_GET['filter']) && empty($_GET['friends']) && empty($_GET['search'])) || ($name[1] == $_GET['filter'] && isset($_GET['filter']) && empty($_GET['r']))) ? ' cover-button-active' : '').'" rel="loadpage" href="'.permalink($this->url.'/index.php?a='.$link).'">'.$value.(($name[2]) ? '<span class="cover-button-value">('.$name[2].')</span>' : '').'</a>';
			}
		}
		...
		foreach($buttons as $value => $name) {
			...
			if($value) {
				...
				$button .= '<a class="'.((($name[1] == $_GET['r'] && empty($_GET['filter'])) || ($name[1] == $_GET['filter'] && isset($_GET['filter']) && empty($_GET['r']))) ? ' profile-menu-active' : '').'" rel="loadpage" href="'.permalink($this->url.'/index.php?a='.$link).'"><div class="message-menu-row">'.$value.(($name[2]) ? ' <span class="profile-menu-value">('.$name[2].')</span>' : '').'</div></a>';
			}
		}
	...
	function checkNewNotifications($limit, $type = null, $for = null, $start = null, $ln = null, $cn = null, $sn = null, $fn = null, $dn = null, $bn = null, $gn = null, $pn = null, $xn = null) {
		global $LNG, $CONF;
		...
		if(!$type) {
			...
			if($ns) {
				...
				// If there are more results available than the limit, then show the Load More Comments
				if(array_key_exists($per_page, $array)) {
					$loadmore = 1;
			...
			if($ns && $loadmore) {
				$events .= sprintf('<div class="load_more" id="more_users"><a onclick="loadNotifications(\'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\')" id="load-more">%s</a></div>', $value['id'], $ln, $cn, $sn, $fn, $dn, $bn, $gn, $pn, $xn, $LNG['view_more_messages']);
			}
			// return the result
			return $events;
		}
			...
			foreach($rows as $comment) {
				...
				<div class="message-reply-container" id="comment'.$comment['id'].'">
					...
					<div class="message-reply-message">
						<span class="message-reply-author"><a href="'.permalink($this->url.'/index.php?a=profile&u='.$comment['username']).'" rel="loadpage">'.$name.'</a></span>: '.$this->parseMessage($comment['message']).'
					</div>
	...
	function getChatType($type, $value, $id) {
		...
		switch($type) {
			case "picture":
			return '<div class="chat-image-thumbnail"><a onclick="gallery(\''.$value.'\', '.$id.', \'media\')" id="'.$value.'"><img src="'.permalink($this->url.'/thumb.php?t=m&w=100&h=100&src='.$value).'"></a></div>';
		}
	}
	...
	function getMessageType($type, $value, $id) {
		...
		switch($type) {
			...
			case "picture":
				...
				if(count($images) == 1) {
					$result .= '<div class="message-type-image event-picture"><a onclick="gallery(\''.$images[0].'\', '.$id.', \'media\')" id="'.$images[0].'"><img src="'.permalink($this->url.'/thumb.php?t=m&w=650&h=300&src='.$images[0]).'"></a>';
				} elseif(count($images) == 2) {
					$result .= '<div class="message-type-image event-picture"><div class="image-container-padding">';
					foreach($images as $image) {
						$result .= '<a onclick="gallery(\''.$image.'\', '.$id.', \'media\')" id="'.$image.'"><div class="image-thumbnail-container-half"><div class="image-thumbnail"><img src="'.permalink($this->url.'/thumb.php?t=m&w=300&h=300&src='.$image).'"></div></div></a>';
					}
					$result .= '</div>';
				} elseif(count($images) == 3) {
					$result .= '<div class="message-type-image event-picture"><div class="image-container-padding">';
					foreach($images as $image) {
						$result .= '<a onclick="gallery(\''.$image.'\', '.$id.', \'media\')" id="'.$image.'"><div class="image-thumbnail-container"><div class="image-thumbnail"><img src="'.permalink($this->url.'/thumb.php?t=m&w=200&h=200&src='.$image).'"></div></div></a>';
					}
					$result .= '</div>';
					...
					foreach($images as $image) {
						if($i == 1) {
							$result .= '<a onclick="gallery(\''.$image.'\', '.$id.', \'media\')" id="'.$image.'"><div class="fake"><div class="image-thumbnail"><img src="'.permalink($this->url.'/thumb.php?t=m&w=650&h=300&src='.$image).'"></div></div></a>';
						} else {
							// If there are more than 4 images, hide them
							if($i > 4) {
								$result .= '<a onclick="gallery(\''.$image.'\', '.$id.', \'media\')" id="'.$image.'" style="display: none;"></a>';
							} else {
								$result .= '<a onclick="gallery(\''.$image.'\', '.$id.', \'media\')" id="'.$image.'"><div class="image-thumbnail-container"><div class="image-thumbnail">'.($i == 4 && count($images) > 4 ? '<span class="image-thumbnail-text">+ '.(count($images)-4).'</span>' : '').'<img src="'.permalink($this->url.'/thumb.php?t=m&w=200&h=200&src='.$image).'"></div></div></a>';
							}
						}
						$i++;
					}
	...
	function delete($id, $type) {
		...
		if($type == 0) {
			...
			// Get the current type (for images deletion)
			$query = $this->db->query(sprintf("SELECT `type`, `value` FROM `chat` WHERE `id` = '%s' AND `from` = '%s'", $this->db->real_escape_string($id), $this->db->real_escape_string($this->id)));
			$message = $query->fetch_assoc();
	...
	function sidebarGender($bold) {
		...
		$link .= '<div class="sidebar-link'.$class.'"><a href="'.permalink($this->url.'/index.php?a='.$_GET['a'].'&q='.htmlspecialchars($_GET['q'], ENT_QUOTES, 'UTF-8').((!empty($_GET['age'])) ? '&age='.htmlspecialchars($_GET['age'], ENT_QUOTES, 'UTF-8') : '')).'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/filters/all.png">'.$LNG["all_genders"].'</a></div>';
	...
	function sidebarSearch($settings) {
		...
		$link .= '<div class="sidebar-link'.$class.'"><a href="'.permalink($this->url.'/index.php?a='.$_GET['a'].'&q='.htmlspecialchars($_GET['q'].$_GET['tag'].$_GET['groups'].$_GET['pages'], ENT_QUOTES, 'UTF-8')).'" rel="loadpage">'.$LNG["sidebar_people"].'</a></div>';
	...
	function sidebarNotifications($bold, $settings = null) {
		...
		$link .= '<div class="sidebar-link'.$class.'"><a href="'.permalink($this->url.'/index.php?a='.$_GET['a']).'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/events/all.png">'.$LNG["recent"].'</a></div>';
		...
		foreach($row as $type) {
			...
			// Output the links
			$link .= '<div class="sidebar-link'.$class.'"><a href="'.permalink($this->url.'/index.php?a='.$_GET['a'].'&filter='.$type).'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/events/'.$type.'.png">'.$LNG["sidebar_{$type}"].'</a></div>';
		}
		$link .= '</div></div>';
		return $link;
	}
	...
	function sidebarTypes($bold) {
		...
		if($row) {
			...
			$link .= '<div class="sidebar-link'.$class.'"><a href="'.permalink($this->url.'/index.php?a='.$_GET['a'].$profile).'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/events/all.png">'.$LNG["all_events"].'</a></div>';
			...
			foreach($row as $type) {
				...
				// Output the links
				$link .= '<div class="sidebar-link sidebar-events'.$class.'"'.$hidden.'><a href="'.permalink($this->url.'/index.php?a='.$_GET['a'].$profile.'&filter='.$type).'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/events/'.$type.'.png">'.$LNG["sidebar_{$type}"].'</a></div>';
		...
		if($rows > 0) {		
			if($for == 0) {
				...
				if($this->profile_data['idu'] && $this->id && $this->id !== $this->profile_data['idu']) {
					...
					#$mutual_text = '<span class="sidebar-header-link"><a href="'.permalink($this->url.'/index.php?a=profile&u='.((!empty($this->profile)) ? $this->profile : $this->username).'&r=friends').'" rel="loadpage">'.$LNG['mutual'].' <span class="sidebar-header-light">('.$mutual.')</span></a></span>';
				}	

				$i = 0;
				$output = '<div class="sidebar-container widget-friends"><div class="sidebar-content"><div class="sidebar-header"><a href="'.permalink($this->url.'/index.php?a=profile&u='.((!empty($this->profile)) ? $this->profile : $this->username).'&r=friends').'" rel="loadpage">'.$LNG['friends'].' <span class="sidebar-header-light">('.$this->friendsCount.')</span></a>'.$mutual_text.'</div><div class="sidebar-padding">';
				foreach($rows as $row) {
	...
	function getChat($uid, $user) {
		...
		$output =	'<div class="message-container">
						<div class="message-content">
							...
							<div class="c-w-icon c-w-icon-smiles" id="chat-smiles-'.$uid.'" onclick="chatPluginContainer('.$uid.', 0, 1)" title="'.$LNG['chat_smiles'].'"></div><label for="chatimage" data-userid="'.$uid.'" class="c-w-icon c-w-icon-picture chat-image-btn" title="'.$LNG['chat_picture'].'"></label><div data-userid="'.$uid.'" class="c-w-icon c-w-icon-camera chat-camera-btn desktop" onclick="cameraModal()" title="'.$LNG['chat_camera'].'"></div>'.$po.'
							</div>
						</div>	
					</div>';
		return $output;
	...
	function getChatMessages($uid = null, $cid = null, $start = null, $type = null, $for = null) {
		...
		if($result = $this->db->query($query)) {
			...
			if($type == 1) {
				...
				$stmt->bind_param('sss', $rows[0]['id'], $this->db->real_escape_string($this->id), $this->db->real_escape_string($uid));
				$stmt->execute();
				$affected = $stmt->affected_rows;
				$stmt->close();
			...
			foreach($rows as $row) {
				...
				<div class="message-reply-container '.$class.'" data-chat-id="'.$row['id'].'" >
					'.$delete.'
					<div class="message-reply-avatar">
						<a href="'.permalink($this->url.'/index.php?a=profile&u='.$row['username']).'" rel="loadpage" title="'.realName($row['username'], $row['first_name'], $row['last_name']).'" id="avatar'.$row['id'].'"><img src="'.permalink($this->url.'/thumb.php?t=a&w=50&h=50&src='.$row['image']).'"></a>
					</div>
	...
	function sidebarPages($visible = null) {
		...
		if(!$visible) {
			$output .= '<div class="sidebar-link"><a href="'.permalink($this->url.'/index.php?a=page').'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/plus.png" width="24" height="24">'.$LNG['create_page'].'</a></div>';
		}
	...
	function sidebarGroups($visible = null) {
		...
		if(!$visible) {
			$output .= '<div class="sidebar-link"><a href="'.permalink($this->url.'/index.php?a=group').'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/plus.png" width="24" height="24">'.$LNG['create_group'].'</a></div>';
		}
	...
	function sidebarBirthdays() {
		...
		$query = $this->db->query(sprintf("SELECT COUNT(*) AS `count`, `username`, `first_name`, `last_name` FROM `users` WHERE EXTRACT(MONTH FROM `born`) = '%s' AND EXTRACT(DAY FROM `born`) = '%s' AND `idu` IN (%s)", date('m'), date('d'), $friendslist));
		
		$result = $query->fetch_assoc();
		
		if($result['count']) {
			return '<div class="sidebar-container widget-birthdays"><div class="sidebar-content"><div class="sidebar-header"><a href="'.permalink($this->url.'/index.php?a=notifications&filter=birthdays').'" rel="loadpage">'.$LNG['friends_birthdays'].'</a></div><div class="sidebar-inner"><div class="sidebar-birthdays"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/n_birthday.png" width="16" height="16">'.(($result['count'] == 1) ? sprintf($LNG['new_birthday_notification'], permalink($this->url.'/index.php?a=notifications&filter=birthdays'), realName($result['username'], $result['first_name'], $result['last_name'])) : sprintf($LNG['x_and_x_others'], permalink($this->url.'/index.php?a=notifications&filter=birthdays'), realName($result['username'], $result['first_name'], $result['last_name']), permalink($this->url.'/index.php?a=notifications&filter=birthdays'), ($result['count']-1))).'</div></div></div></div>';
		}
	}
	...
	function getGroups($start = null, $value = null, $live = null) {
		...
		if(empty($value)) {
			...
			if(empty($live)) {
				...
				$button = $LNG['view'];
				$link = permalink($this->url.'/index.php?a=group&name='.$row['name']);
		...
		if($live) {
				...
				$output .= '<div class="'.$class.'" id="group'.$row['id'].'">
								<div class="message-content">
									<div class="message-inner">
										...
										<div class="message-avatar">
											<a href="'.permalink($this->url.'/index.php?a=group&name='.$row['name']).'" rel="loadpage">
												<img src="'.($admin ? $this->url.'/thumb.php?t=c&w=48&h=48&src='.$row['cover'] : permalink($this->url.'/thumb.php?t=c&w=48&h=48&src='.$row['cover'])).'">
											</a>
										</div>
										<div class="message-top">
											<div class="message-author">
			...
			if($loadmore) {
					$output .= '<div class="load_more" id="'.(empty($value) ? 'more_users' : 'more_messages').'"><a onclick="group('.(empty($value) ? 5 : 3).', \''.htmlspecialchars($value).'\', \''.$this->db->real_escape_string($start + $this->per_page).'\', \''.$this->profile_data['idu'].'\')" id="load-more">'.$LNG['view_more_messages'].'</a></div>';
			}
		}
		return $output;
	}
				...
				$output .= '<div class="'.$class.'" id="page'.$row['id'].'">
								<div class="message-content">
									<div class="message-inner">
										...
										<div class="message-avatar">
											<a href="'.permalink($this->url.'/index.php?a=page&name='.$row['name']).'" rel="loadpage">
												<img src="'.($admin ? $this->url.'/thumb.php?t=a&w=48&h=48&src='.$row['image'] : permalink($this->url.'/thumb.php?t=a&w=48&h=48&src='.$row['image'])).'">
											</a>
										</div>
										<div class="message-top">
											<div class="message-author">
	...
	function createPage($values, $type = null) {
		...
		$values['page_phone'] = substr(htmlspecialchars($values['page_phone']), 0, 64);
		$values['page_address'] = substr(htmlspecialchars($values['page_address']), 0, 128);
		$values['page_website'] = substr($values['page_website'], 0, 128);
...
function admin_stats($db, $type, $values = null, $category = null, $extra = null) {
	...
	if($type == 2) {
		...
		if($extra == 2) {
			...
			$queries .= sprintf("(SELECT count(`id`) FROM `likes` WHERE `time` >= '%s' AND `time` < '%s') as likes,", $start, $end, $value);
...
function permalink($url) {
	...
	if($settings['permalinks']) {
		...
		if(strpos($url, $path['profile'])) {
			...
			$url = str_replace(array($path['feed'], '&filter=', '&logout=1', '&token_id='), array('feed', '/filter/', '/logout/', ''), $url);
